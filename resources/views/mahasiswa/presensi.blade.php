    ...
    <table class="w-full text-left border-collapse">
        <thead>
            <tr class="border-b border-slate-200 bg-slate-50 text-slate-500 text-[11px] font-bold uppercase tracking-wider">
                <th class="px-6 py-4">Mata Kuliah Kulikulum</th>
        ...
        <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
            @forelse($absensis as $absen)
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-center font-bold text-slate-600">Pertemuan Ke-{{ $absen->pertemuan_ke }}</td>
                    <td class="px-6 py-4 text-center text-xs font-mono text-slate-500">{{ \Carbon\Carbon::parse($absen->tanggal)->translatedFormat('d M Y') }} | {{ substr($absen->jam, 0, 5) }} WIB</td>
