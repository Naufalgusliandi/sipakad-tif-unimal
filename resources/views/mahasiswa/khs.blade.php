...
<div class="space-y-6">
    ...
    <div class="flex justify-end print-hide">
        <button onclick="window.print()" class="px-5 py-2.5 text-white font-bold text-xs rounded-xl shadow-md flex items-center gap-2 transition-all transform active:scale-95 hover:shadow-lg" style="background: linear-gradient(135deg, #047857, #059669) !important; color: #ffffff !important; border: none !important;">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm print-target p-2">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50 text-slate-500 text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4 text-center bg-emerald-50 text-emerald-800">Nilai Akhir</th>
                    <th class="px-6 py-4 text-center bg-amber-50 text-amber-800">Huruf</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                @forelse($nilais as $nilai)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center font-mono text-xs">{{ number_format($nilai->tugas, 1) }}</td>
                        <td class="px-6 py-4 text-center font-mono text-xs">{{ number_format($nilai->quiz, 1) }}</td>
                        <td class="px-6 py-4 text-center font-mono text-xs">{{ number_format($nilai->uts, 1) }}</td>
                        <td class="px-6 py-4 text-center font-mono text-xs">{{ number_format($nilai->uas, 1) }}</td>
                        <td class="px-6 py-4 text-center bg-emerald-50/40 font-bold font-mono text-emerald-700">{{ number_format(($nilai->tugas*0.2)+($nilai->quiz*0.15)+($nilai->uts*0.3)+($nilai->uas*0.35), 2) }}</td>
                        <td class="px-6 py-4 text-center bg-amber-50/40 font-extrabold text-amber-700">
        ...
        <div class="p-6 bg-slate-50/80 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 rounded-xl">
            ...
            <div class="flex items-center gap-4">
                ...
                <!-- Mengambil variabel IPK dari Controller -->
