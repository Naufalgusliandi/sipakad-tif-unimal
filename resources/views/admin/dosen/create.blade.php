...
<div class="bg-slate-900/20 border border-slate-800/60 backdrop-blur-xl rounded-2xl p-5 sm:p-8 max-w-4xl">
    ...
    <form action="{{ route('admin.dosen.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-800/60">
            <a href="{{ route('admin.dosen.index') }}" class="w-full sm:w-auto text-center px-5 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs font-semibold text-slate-400 hover:text-white transition-colors">Batal</a>
<button type="submit" class="w-full sm:w-auto px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-xs font-semibold rounded-xl shadow-md shadow-purple-600/10 hover:from-purple-500 hover:to-indigo-500 transition-all">Simpan Data Dosen</button>
        </div>
    </form>
</div>
