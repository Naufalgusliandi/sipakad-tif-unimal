...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    ...
    <a href="{{ route('admin.dosen.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl text-sm font-semibold shadow-lg shadow-indigo-600/15 hover:from-indigo-500 hover:to-purple-500 transition-all">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-800 bg-slate-900/40 text-slate-400 text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4">Jabatan Akademik</th>
            ...
            <tbody class="divide-y divide-slate-800/60 text-sm text-slate-300">
                @forelse($dosens as $dsn)
                    <tr class="hover:bg-slate-900/30 transition-colors group">
                        ...
                        <td class="px-6 py-4 font-mono text-xs text-slate-400 tracking-wider">{{ $dsn->nidn }}</td>
                        <td class="px-6 py-4">
