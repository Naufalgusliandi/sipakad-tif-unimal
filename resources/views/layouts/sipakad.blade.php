...
<body class="bg-slate-100 text-slate-900 min-h-screen overflow-x-hidden relative selection:bg-emerald-500/20" x-data="{ open: false }">

    <div class="flex min-h-screen overflow-hidden">
        
        <!-- BACKDROP LAYAR MOBILE -->
        <div x-show="open" @click="open = false" class="fixed inset-0 bg-slate-900/60 z-40 lg:hidden backdrop-blur-sm" x-cloak></div>

        <!-- 1. SIDEBAR KHUSUS MOBILE (DRAWER SLIDE-IN) -->
        <aside :class="open ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 w-72 bg-emerald-950 border-r border-emerald-900 p-6 flex flex-col justify-between shrink-0 z-50 lg:hidden shadow-xl transition-transform duration-300 ease-in-out overflow-y-auto custom-scrollbar">
            <div>
                ...
                <nav class="space-y-1.5">
                    ...
                    @if(auth()->user()->role === 'admin')
                        ...
                        <a href="{{ route('admin.mahasiswa.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.mahasiswa.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Mahasiswa</a>
                        <a href="{{ route('admin.dosen.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.dosen.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Dosen</a>
                        <a href="{{ route('admin.mata-kuliah.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.mata-kuliah.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Mata Kuliah</a>
                        <a href="{{ route('admin.kelas.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.kelas.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Kelas</a>
                        <a href="{{ route('admin.ruangan.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.ruangan.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Ruangan</a>
                        <a href="{{ route('admin.jadwal.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.jadwal.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Jadwal Kuliah</a>
                    @endif
                    ...
                    @if(auth()->user()->role === 'dosen')
                        ...
                        <a href="{{ route('dosen.absensi.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('dosen.absensi.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Kelola Absensi</a>
                        <a href="{{ route('dosen.nilai.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('dosen.nilai.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Kelola Nilai</a>
                        <a href="{{ route('dosen.materi.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('dosen.materi.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Kelola Materi</a>
                    @endif
                    ...
                    @if(auth()->user()->role === 'mahasiswa')
                        ...
                        <a href="{{ route('mahasiswa.presensi') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('mahasiswa.presensi') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Lihat Presensi</a>
                        <a href="{{ route('mahasiswa.khs') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('mahasiswa.khs') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Kartu Hasil Studi (KHS)</a>
                        <a href="{{ route('mahasiswa.materi.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('mahasiswa.materi.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Unduh Materi Kuliah</a>
                        <a href="{{ route('mahasiswa.krs.index') }}" class="flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('mahasiswa.krs.*') ? 'bg-amber-500 text-emerald-950 font-bold shadow-md' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">Isi KRS Manual</a>
                    @endif
                </nav>
            </div>
        ...
        <!-- 2. SIDEBAR KHUSUS DESKTOP (ASLI & NORMAL FLOW) -->
        <aside class="w-72 bg-emerald-950 border-r border-emerald-900 p-6 flex flex-col justify-between hidden lg:flex shrink-0 z-20 shadow-xl shadow-slate-900/5 min-h-screen">
            <div>
                <div class="flex items-center gap-3.5 px-2 mb-8">
                    <div class="h-11 w-11 bg-white p-1 rounded-xl flex items-center justify-center shadow-md border border-slate-100 shrink-0">
                        <img src="{{ asset('images/logo-tif.png') }}" alt="Logo TIF" class="w-full h-full object-contain">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen bg-slate-100">
            ...
            <header class="h-20 border-b border-slate-200 bg-white flex items-center justify-between px-4 sm:px-8 shrink-0 z-10 shadow-sm">
                <div class="flex items-center gap-3">
                    <!-- Tombol Hamburger Mobile -->
                    <button @click="open = !open" class="lg:hidden p-2 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200">
                    ...
                    <div>
                        ...
                        <h1 class="text-base sm:text-xl font-extrabold text-slate-900 tracking-tight mt-0.5" style="color: #0f172a !important;">@yield('page_title', 'Dashboard')</h1>
                    </div>
                </div>
                
                <div class="flex items-center gap-2 sm:gap-4">
                    <div class="hidden sm:block text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-200/60 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl tracking-wide font-mono" style="color: #064e3b !important;">
